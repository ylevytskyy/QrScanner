...
struct QRProcessor::Implementation {
 public:
  ...
  void process(const Mat &image) {
    ...
    *traces = Scalar(0, 0, 0);
    ...
    // 2. Alternately, the Ratio of areas of the "concentric" squares can also
    // be used for identifying base Alignment markers.
    ...
    for (int i = 0; i < contours.size(); ++i) {
      ...
      if (c >= 5) {
        if (mark == 0) {
          A = i;
        } else if (mark == 1) {  // i.e., A is already found, assign current
                                 // contour to B
          B = i;
        } else if (mark == 2) {  // i.e., A and B are already found, assign
                                 // current contour to C
          C = i;
        }
        mark = mark + 1;
      }
    ...
    if (mark >= 2) {  // Ensure we have (atleast 3; namely A,B,C) 'Alignment
                      // Markers' discovered
      ...
      // Now that we have the orientation of the line formed median1 & median2
      // and we also have the position of the outlier w.r.t. the line
      ...
      if (top < contours.size() && right < contours.size() &&
        ...
        foundCross =
            getIntersectionPoint(rightPoints[1], rightPoints[2],
                                 bottomPoints[3], bottomPoints[2], cross);
        ...
        if (src.size() == 4 && dst.size() == 4) {  // Failsafe for WarpMatrix
                                                   // Calculation to have only 4
                                                   // Points with src and dst
          warp_matrix = getPerspectiveTransform(src, dst);
          warpPerspective(image, qr_raw, warp_matrix,
                          cv::Size(qr.cols, qr.rows));
          copyMakeBorder(qr_raw, qr, 10, 10, 10, 10, BORDER_CONSTANT,
                         Scalar(255, 255, 255));
        ...
        if (debugEnabled) {
          ...
          if (slope > 5) {
            ...
            circle(*traces, cv::Point(10, 20), 5, Scalar(255, 255, 255), -1, 8,
                   0);
          }

          // Draw contours on Trace image for analysis
          drawContours(*traces, contours, top, Scalar(255, 0, 100), 1, 8,
                       hierarchy, 0);
          drawContours(*traces, contours, right, Scalar(255, 0, 100), 1, 8,
                       hierarchy, 0);
          drawContours(*traces, contours, bottom, Scalar(255, 0, 100), 1, 8,
                       hierarchy, 0);

          // Draw points (4 corners) on Trace image for each Identification
          // marker
          ...
          if (orientation == CV_QR_NORTH) {
            putText(*traces, "NORTH", cv::Point(20, 30), fontFace, 1,
                    Scalar(0, 255, 0), 1, 8);
          } else if (orientation == CV_QR_EAST) {
            putText(*traces, "EAST", cv::Point(20, 30), fontFace, 1,
                    Scalar(0, 255, 0), 1, 8);
          } else if (orientation == CV_QR_SOUTH) {
            putText(*traces, "SOUTH", cv::Point(20, 30), fontFace, 1,
                    Scalar(0, 255, 0), 1, 8);
          } else if (orientation == CV_QR_WEST) {
            putText(*traces, "WEST", cv::Point(20, 30), fontFace, 1,
                    Scalar(0, 255, 0), 1, 8);
          }
    ...
    callback(callbackData, originalImage, *traces, qr_thres, topPoint,
             bottomPoint, rightPoint, cross, foundCross, orientation);
  }
 ...
 private:
  ...
  static float cv_distance(Point2f P, Point2f Q) { return cv::norm(P - Q); }

  // Function: Perpendicular Distance of a Point J from line formed by Points L
  // and M; Equation of the line ax+by+c=0
  // Description: Given 3 points, the function derives the line quation of the
  // first two points,
  //	  calculates and returns the perpendicular distance of the the 3rd point
  //from this line.
  ...
  // Function: Slope of a line by two Points L and M on it; Slope of line, S =
  // (x1 -x2) / (y1- y2)
  // Description: Function returns the slope of the line formed by given 2
  // points, the alignement flag
  ...
  // Function: Routine to calculate 4 Corners of the Marker in Image Space using
  // Region partitioning
  // Theory: OpenCV Contours stores all points that describe it and these points
  // lie the perimeter of the polygon.
  //	The below function chooses the farthest points of the polygon since they
  //form the vertices of that polygon,
  //	exactly the points we are looking for. To choose the farthest point, the
  //polygon is divided/partitioned into
  //	4 regions equal regions using bounding box. Distance algorithm is
  //applied between the centre of bounding box
  //	every contour point in that region, the farthest point is deemed as the
  //vertex of that region. Calculating
  //	for all 4 regions we obtain the 4 corners of the polygon ( -
  //quadrilateral).
  static void cv_getVertices(vector<vector<cv::Point> > contours, int c_id,
                             float slope, vector<Point2f> &quad) {
    cv::Rect box;
    ...
    if (slope > 5 || slope < -5) {
      for (int i = 0; i < contours[c_id].size(); ++i) {
        pd1 = cv_lineEquation(
            C, A,
            contours[c_id][i]);  // Position of point w.r.t the diagonal AC
        pd2 = cv_lineEquation(
            B, D,
            contours[c_id][i]);  // Position of point w.r.t the diagonal BD
  ...
  // Function: Compare a point if it more far than previously recorded farthest
  // distance
  // Description: Farthest Point detection using reference point and baseline
  // distance
  static void cv_updateCorner(Point2f P, Point2f ref, float &baseline,
                              Point2f &corner) {
    float temp_dist;
  ...
  static void cv_updateCornerOr(int orientation, vector<Point2f> IN,
                                vector<Point2f> &OUT) {
    Point2f M0, M1, M2, M3;
    if (orientation == CV_QR_NORTH) {
      M0 = IN[0];
  ...
  // Function: Get the Intersection Point of the lines formed by sets of two
  // points
  static bool getIntersectionPoint(Point2f a1, Point2f a2, Point2f b1,
                                   Point2f b2, Point2f &intersection) {
    Point2f p = a1;
    Point2f q = b1;
    Point2f r(a2 - a1);
    Point2f s(b2 - b1);
...
QRProcessor::~QRProcessor() { delete pThis; }

bool QRProcessor::startCapture() { return pThis->startCapture(); }

void QRProcessor::process() { pThis->process(); }

void QRProcessor::process(const cv::Mat &image) { pThis->process(image); }
